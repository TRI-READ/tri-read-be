...
public class AdminQuizController {
    ...
    @PostMapping public ResponseEntity<AdminQuizService.QuizDetail> create(@Valid @RequestBody CreateQuizRequest request) {
        AdminQuizService.CreateQuiz command = new AdminQuizService.CreateQuiz(request.challengeDate(), request.passages().stream().map(p ->
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(service.createDraft(command));
    }
