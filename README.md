...
- 평일마다 서로 다른 영역의 지문 3개와 지문별 객관식 3문제를 필수 학습으로 제공합니다.
...
기본 생성 스케줄은 매일 새벽에 오늘부터 향후 3일의 재고를 확인하고, 30분 뒤 한 차례 더 부족분을 복구합니다. 주말 퀴즈도 선택 보충 학습을 위해 같은 방식으로 준비합니다. 시간은 `QUIZ_GENERATION_CRON`, `QUIZ_GENERATION_RECOVERY_CRON`으로 조정합니다. 각 날짜는 최대 3세트를 보유하며 사용되지 않은 기존 발행 문제를 우선 재배정해 Gemini 호출을 줄입니다.
