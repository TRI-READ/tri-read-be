...
- 매일 서로 다른 영역의 지문 3개와 지문별 객관식 3문제를 제공합니다.
- 사용자는 원하는 지문 하나를 골라 3문제를 풀면 오늘의 필수 학습을 완료합니다.
- 필수 학습 완료 후 남은 지문 2개는 보너스로 선택해 풀 수 있습니다.
- 발행된 퀴즈는 콘텐츠 풀에 쌓이며, 사용자가 아직 보지 않은 오래된 세트부터 학습일에 고정 배정됩니다.
- 기본 지문을 시작하지 않은 세트는 다음 접속일로 이월되고, 기본 학습을 마친 세트의 남은 지문은 보너스 서재에 유지됩니다.
- 주말에도 별도 퀴즈를 제공하지만 학습은 강제가 아니며, 완료 기록은 해당 주의 비어 있는 평일을 가장 이른 날짜부터 채웁니다.
...
- [분류별 트러블슈팅 기록](docs/troubleshooting/README.md)
...
# 퀴즈
POST /api/quizzes/today
GET  /api/quizzes/bonus
...
퀴즈 제출 요청은 한 지문에 속한 정확히 3개의 답을 포함해야 합니다. 학습일에 배정된 세트의 첫 제출은 `PRIMARY`, 이후 같은 세트의 다른 지문은 `BONUS`로 저장됩니다.

`PRIMARY` 제출 하나가 그날의 필수 학습과 스트릭을 완료합니다. `BONUS` 제출은 추가 학습 기록으로만 저장되며 스트릭을 중복 증가시키지 않습니다. 당일 남은 지문과 과거 기본 학습에서 남은 지문은 보너스로 이어 풀 수 있습니다. 한 세트의 지문은 각각 한 번만 제출할 수 있습니다.
...
기본 생성 스케줄은 매일 새벽에 오늘부터 향후 3일의 콘텐츠 생산 여부를 확인하고, 30분 뒤 한 차례 더 부족분을 복구합니다. 기본값은 날짜마다 새 세트 1개이며 `QUIZ_SETS_PER_DATE`로 조정합니다. 발행된 세트는 날짜가 지나도 폐기하거나 다른 날짜로 이동하지 않고 콘텐츠 풀에 남으며, 사용자별 미열람 순서로 계속 배정됩니다.
